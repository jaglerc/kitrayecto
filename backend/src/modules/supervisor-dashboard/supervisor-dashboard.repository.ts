...
interface DocumentRow { id: string; document_type: string; plate: string; expires_at: Date; days_remaining: number; }
        ...
        const [users, vehicles, alerts, documents] = await Promise.all([
            ...
            pool.query<DocumentRow>(
                `SELECT * FROM (
                    ...
                    FROM seguros_vehiculos s JOIN vehiculo v ON v.id = s.carro_id
                    ...
                    SELECT 'tecnomecanica-' || r.id AS id, 'Revisión técnico-mecánica' AS document_type,
                           v.placa AS plate, r.fecha_expiracion_tecnicomecanica AS expires_at,
                           (r.fecha_expiracion_tecnicomecanica::date - CURRENT_DATE)::integer AS days_remaining
                    FROM revisiones_tecnicomecanicas r JOIN vehiculo v ON v.id = r.carro_id
                    WHERE r.fecha_expiracion_tecnicomecanica::date BETWEEN CURRENT_DATE AND CURRENT_DATE + 30
                 ) expiring ORDER BY expires_at ASC LIMIT 8`
            ),
        ]);
        const mappedDocuments: ExpiringDocument[] = documents.rows.map((row) => ({ id: row.id, documentType: row.document_type, plate: row.plate, expiresAt: row.expires_at.toISOString(), daysRemaining: row.days_remaining }));
